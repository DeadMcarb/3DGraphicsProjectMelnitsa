...
public class VideoCardBoard {
    ...
    public VideoCardBoard(int x, int y, int z, int xAngle, int yAngle, int zAngle, GroupExtended group, boolean on) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.xAngle = xAngle;
    ...
    public void init(){
        ...
        Panel board = new Panel(500, 240, 8, Color.rgb(64, 54, 52), on);
        videoCardBoard.getChildren().add(board.getPanel());

        Panel  pci = new Panel(190, 50, 8, Color.rgb(64, 54, 52), on);
        ...
        for (int i = 1; i <= 16; i++) {
            Panel p1 = new Panel(14, 7, 6, Color.GOLD, on);
        ...
        Chip chipset = new Chip(0, 110, 7, 30, 30, 0, 0, 0, videoCardBoard, Color.BLACK, on);
        chipset.init();
        Chip chipset2 = new Chip(50, 110, 7, 30, 30, 0, 0, 0, videoCardBoard, Color.BLACK, on);
        chipset2.init();
        Chip chipset3 = new Chip(100, 110, 7, 30, 30, 0, 0, 0, videoCardBoard, Color.BLACK, on);
        chipset3.init();

        Panel left_plane = new Panel(70, 270, 10, Color.rgb(228, 228, 228), on);
        ...
        Panel top_left_plane = new Panel(50, 70, 5, Color.rgb(228, 228, 228), on);
